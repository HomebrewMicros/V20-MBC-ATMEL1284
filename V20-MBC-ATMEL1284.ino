...
void setup() 
{
    ...
    if ((bootSelection == 1) || (bootMode > maxBootMode))
    {
        ...
        {
            bootMode = EEPROM.read(bootModeAddr);  // Reload boot mode if it was selected '0' or > '5' (system parameter           
        }
        //  or no change)
    }
        ...
        case 4:  // Load an OS from current Disk Set on SD
            switch (diskSet) 
            {
                ...
                case 1:  // CP/M-86
                    ...
                    //
                    fileNameSD = CPM86FN;
                    JumpStrAddr = CPM86JMPADDR;     // Starting address of the first instruction to execute ("far" jump)
                    V20initJumpSegm = CPM86JMPSEG;  // Segment used for the first instruction to execute ("far" jump)
    ...
    if (debug > 0) 
    {
        Serial.println(F("* * * LOAD DS 0xFFFF * * *\r\n"));
    }        
    loadDS(0xffff);
    ...
    if (debug > 0) 
    {
        Serial.println(F("* * * STORE JMPF 0x0000:V20initJumpAddr * * *\r\n"));
    }        
    ...
    if (debug > 0) 
    {
        Serial.println(F("* * * LOAD DS loadSegment * * *\r\n"));
    }        
    loadDS(loadSegment);
    ...
    if (autoboot80)
    {
        ...
        // Store the previously loaded 8080 code starting address into the corresponding Switch-80 data area
        storeOneByte(lowByte(FirstStageStrAddr), SWITCH80_STRADDR);
        storeOneByte(highByte(FirstStageStrAddr), SWITCH80_STRADDR + 1);

        // Store the previously loaded 8080 code size (in bytes) into the corresponding Switch-80 data area
    ...
    if (debug > 0) 
    {
        Serial.println(F(" * V20 CPU 2nd RESET Done * \r\n"));
    }
...
void loop() 
{
    ...
    {
        // I/O , interrupt operation requested, or CPU HALT
        if ((PIN_WR_) == 0)  // WR_ = 0 ?
        {
    ...
    if (Serial.available() && V20IntEnFlag) 
    {
        digitalWrite(INTR, HIGH);  // Generate an interrupt on serial Rx if enabled
    }        
}
...
void openFileSD(const char *fileNameSD)
{
    ...
    {
        // Error mounting. Try again    errCodeSD = mountSD(&filesysSD);
        if (errCodeSD) 
        {
...
//
// Returns the number of loaded bytes. If the file is 65536 bytes long the returned size is 0.
//
...
word loadSDfileToRAM(word BootStrAddr) 
{
    ...
    // If an error occurs repeat until error disappears (or the user forces a reset)
    {
        ...
        // Read a "segment" of a SD sector and load it into RAM
        {
            ...
            if (numReadBytes > 0) 
            {
                emptyFile = 0;          // Check for an empty file
            }
            ...
            for (iCount = 0; iCount < numReadBytes; iCount++)
            {
                ...
                if (loadAddress == 0) 
                {
                    overRun = 1;  // Reached the end of the 64KB address space
                }                    
            }
